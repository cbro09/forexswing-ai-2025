...
class ForexTrainingPipeline:
    ...
    def load_market_data(self):
        ...
        # Load EUR/USDT and BTC/USDT data (Feather format)
        all_data = []
        
        for pair in ['EUR_USDT-4h.feather', 'BTC_USDT-4h.feather']:
            file_path = os.path.join(data_dir, pair)
            ...
            if os.path.exists(file_path):
                ...
                df['pair'] = pair.replace('-4h.feather', '').replace('_', '/')
                all_data.append(df)
        ...
        if not all_data:
            raise ValueError("No market data found! Run download-data first.")
    ...
    def train_model(self, epochs=50, batch_size=32, learning_rate=0.001):
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                # Save best model
                torch.save(model.state_dict(), '../models/forex_lstm_best.pth')
            else:
                patience_counter += 1
